...
import { maskCPF, maskNumbersOnly, maskPhone } from '../../lib/utils';
...
export const Step1Personal: React.FC = () => {
  const { register, formState: { errors }, setValue, watch } = useFormContext<OnboardingFormData>();
  ...
  useEffect(() => {
    if (phoneValue) setValue("phone", maskPhone(phoneValue));
  }, [phoneValue, setValue]);

  useEffect(() => {
      if (rgValue) setValue("rg", maskNumbersOnly(rgValue));
  }, [rgValue, setValue]);
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <FormItem>
          ...
          <FormMessage message={errors.dob?.message} />
        </FormItem>
        ...
        <FormItem className="md:col-span-2">
          ...
          <FormMessage message={errors.cpf?.message} />
        </FormItem>
        
        {/* RG */}
        <FormItem className="md:col-span-2">
          <Label htmlFor="rg">RG</Label>
          <Input id="rg" placeholder="Apenas números" {...register("rg")} />
          <FormMessage message={errors.rg?.message} />
        </FormItem>

        {/* Issuing Authority */}
        <FormItem className="md:col-span-2">
            <Label htmlFor="issuingAuthority">Órgão Emissor</Label>
            <Select id="issuingAuthority" {...register("issuingAuthority")}>
                <option value="">Selecione</option>
                {SSP_LIST.map((item) => (
                    <option key={item.sigla} value={`${item.sigla} - ${item.extenso}`}>
                        {item.sigla} - {item.extenso}
                    </option>
                ))}
            </Select>
            <FormMessage message={errors.issuingAuthority?.message} />
        </FormItem>
      </div>
    </div>
  );
